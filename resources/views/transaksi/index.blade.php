...
<section class="content">
      ...
      <!-- Info boxes -->
      <div class="col-12">
        <a href="{{ route('transaksi.cetak') }}" class="btn btn-success mb-2">Cetak PDF</a>
        <div class="card">
                ...
                <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td><span class="badge badge-{{($item->logpaket_id !== null) ? 'warning':'primary'}}">{{ ($item->logpaket_id !== null) ? 'Pembayaran Upgrade paket':'Belum Aktif 1 bulan' }}</span></td>
                        {{-- @if ($item->logpaket_id !== null)
                        <td><span class="badge badge-warning">Tagihan ubah paket</span></td>
                        @endif --}}
                        @else
                        <td>{{ $item->bulantempo->bulan }} - {{ $item->bulantempo->tahun }}</td>
                        @endif
                        {{-- @if ($item->tanggaltempo_id == null)
                        <td><span class="badge badge-primary">Belum Aktif 1 bulan</span></td>
                        @elseif ($item->logpaket_id !== null)
                        <td>Tagihan ubah paket</td>
                        @else
                        <td>{{ $item->bulantempo->bulan }} - {{ $item->bulantempo->tahun }}</td>
                        @endif --}}
                        {{-- @if ($item->tanggaltempo_id == null)
